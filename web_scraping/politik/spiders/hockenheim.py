...
class HockenheimSpider(scrapy.Spider):
    name = 'hockenheim'

    custom_settings = {
        'FILES_STORE': 'Dateien/Kommunen/Hockenheim'
    }

    urls = []

    for date in range (2018,2023):
        crawl_url = f"https://session.hockenheim.de/bi/si0046.asp?__cjahr={date}&__cmonat=1&__canz=12&__ksigrnr=1&__osidat=d&smccont=85&__cselect=65536"
    ...
    def parse(self, response):
        ...
        base_url = 'https://session.hockenheim.de/bi/'
    ...
    def parse_sitzungen(self, response):
        ...
        for row in file_table:
            ...
            if row.xpath("td/div/div/div/div/div/a/@href").get() is not None:
                ...
                top_name = ' '.join(row.xpath("td[@class='tolink']/div/text()").getall())
                if top_name == '':
