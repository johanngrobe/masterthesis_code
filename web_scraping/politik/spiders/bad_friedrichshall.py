...
class BadFriedrichshallSpider(scrapy.Spider):
    name = "bad_friedrichshall"
    path = '//home/johanngrobe/python_env/politik/Websites/Friedrichshall'

    custom_setting = {
        'FILES_STORE': 'Dateien/Kommune/Bad_Friedrichshall'
    ...
    def start_requests(self):
        ...
        for file in os.listdir(self.path):
            ...
            if os.path.splitext(file)[-1] == '.htm':
                    url = f"file:///home/johanngrobe/python_env/politik/Websites/Friedrichshall/{file}"
                    yield scrapy.Request(url=url, callback=self.parse)

        # url = 'file:///home/johanngrobe/python_env/politik/Websites/Friedrichshall/Friedrichshall_20201.htm'
        # yield scrapy.Request(url=url, callback=self.parse)
    ...
    def parse(self, response):
        ...
        sitzungen = response.xpath("//div[@class='calendar-page-day-meeting']/a")

        for sitzung in sitzungen:
            gremium = sitzung.css("div.calendar-page-day-meeting-title::text").get()
            if 'Gemeinderat' in gremium:
    ...
    async def parse_sitzungen(self, response):
        ...
        kommune_clean = 'Bad Friedrichshall'
    
        kommune = 'Bad_Friedrichshall'
        ...
        if gen_doc_header is not None:
            if 'Dokumente' in gen_doc_header:
                ...
                for doc, doc_pw in zip(gen_doc, await meeting_attachments.all()):
                    ...
                    if doc.css("svg::attr(data-icon)").get() == 'file-pdf':
                        doc_name = ' '.join(doc.css("div.highlight-wrapper::text").getall())
        ...
        for top, top_pw in zip(tops, await tops_pw.all()):
            ...
            if len(top.css("div.highlight-wrapper")) > 0:
                ...
                top_name = top.css("div.agenda-item-title-content div::text").get()

                # Main doc
                main_doc = top.xpath(".//div[@class='document-attachment-content']")
                main_doc_pw = top_pw.locator("div.document-attachment-name[role='button']")
                for doc, doc_pw in zip(main_doc, await main_doc_pw.all()):
                    doc_name = doc.xpath(".//div/text()").get()
                    doc_typ = doc_name
                ...
                attachments_pw = top_pw.locator("div.agenda-item-documents-attachment")
                for attachment, attachment_pw in zip(attachments, await attachments_pw.all()):
                    doc_name = attachment.xpath(".//div/text()").get()
                    doc_typ = 'TOP Anhang'
