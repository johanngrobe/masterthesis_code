...
class SpeyerSpider(scrapy.Spider):
    name = 'speyer'

    custom_settings = {
        'FILES_STORE': 'Dateien/Kommunen/Speyer'
    }

    urls = []
  
    for date in range (2005,2023):
        crawl_url = f"https://buergerinfo2.speyer.de/si0046.php?__cjahr={date}&__cmonat=1&__canz=12&smccont=85&__osidat=d&__ksigrnr=1&__cselect=65536"
    ...
    def parse(self, response):
        base_url = 'https://buergerinfo2.speyer.de/'
        sitzungen = response.xpath("//a[@class='smc-link-normal smc_doc smc_datatype_si']/@href").getall()
    ...
    def parse_sitzungen(self, response):
        ...
        generel_documents = response.xpath("//div[@class='smc-el-h smc-dg-a2 smc-t-r991']/a") 
        ...
        for row in file_table:
            ...
            if row.xpath("td/div/div/div/div/div/a/@href").get() is not None:
                ...
                files = row.xpath("td/div/div/div/div/div/a[@class='smc-dg-a3 smc-text-block-991 smc-t-r991']")
