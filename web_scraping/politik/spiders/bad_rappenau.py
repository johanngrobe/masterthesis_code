...
class BadRappenauSpider(scrapy.Spider):
    name = 'bad_rappenau'

    custom_settings = {
        'FILES_STORE': 'Dateien/Kommunen/Bad_Rappenau'
    }

    urls = []
  
    for date in range (2016,2023):
        crawl_url = "https://buergerinfo.badrappenau.de/si0046.asp?__cjahr={}&__cmonat=1&__canz=12&__osidat=d&smccont=85&__kgsgrnr=1&__cselect=65536".format(date)
        urls.append(crawl_url)
    
    start_urls = urls

    def parse(self, response):
        base_url = 'https://buergerinfo.badrappenau.de/'
    ...
    def parse_sitzungen(self, response):
        ...
        kommune = 'Bad_Rappenau'
        kommune_clean = 'Bad Rappenau'
        ...
        for row in file_table:
            ...
            if row.xpath("td/div/div/div/div/div/a/@href").get() is not None:
                ...
                top_name = row.xpath("td[@class='tobetr']/div/text()").get()
